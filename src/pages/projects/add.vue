    ...
    <div class="glass-effect-lg rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
      <div class="flex items-center justify-between px-8 py-6 border-b border-gray-200/50 bg-gradient-to-r from-white/50 to-white/30">
        <div class="flex items-center gap-4">
          ...
          <h1 class="text-xl font-semibold text-gray-900">{{ t('projects.addTitle') }}</h1>
        </div>
        <button
          @click="handleSave"
...
const { terminalToolPresets } = storeToRefs(projectStore)
const { providers } = storeToRefs(providerStore)

const form = ref<ProjectFormState>(createDefaultProjectForm())
...
onMounted(async () => {
  ...
  form.value = createDefaultProjectForm(terminalToolPresets.value)
